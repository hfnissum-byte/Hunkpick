...
import { conflictedPaths, mergeContext, repoRoot } from "./lib/git.mjs";
import { render } from "./lib/conflicts.mjs";
...
async function main() {
  ...
  const { files, items, skipped } = collectFiles(paths, root, opts);

  const total = countDecisions(files);
  if (!total) {
