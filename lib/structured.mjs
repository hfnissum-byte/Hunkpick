/**
 * Three-way merge over object keys.
 *
 * Two branches adding different keys to package.json cannot be resolved by a
 * line or token merge, because the result needs a comma present in neither
 * version. Merging the parsed objects and serialising once avoids that.
 *
 * Only applied when all three stages parse. Anything else falls back to the
 * line-level path.
 */
...
export function structuredMerge(ext, baseText, oursText, theirsText) {
  ...
  // Keys the merge added relative to the current branch, for the report.
  const added = [];
  const walk = (a, b, path) => {
