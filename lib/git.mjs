/**
 * Everything that touches git. Nothing here writes to the working tree except
 * writeResolved, and nothing here ever stages or commits — leaving the result
 * for the user to review is the point.
 */
 ...
 *
 * Returns null when a stage is missing (add/add, delete/modify and friends) —
 * those are not ours to resolve.
 */
export function diff3Text(path, cwd) {
  ...
  const dir = mkdtempSync(join(tmpdir(), "jevmerge-"));
  try {
    const f = (name, text) => {
