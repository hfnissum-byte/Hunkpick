/**
 * Enumerate every resolution a conflict could legitimately have.
 *
 * The whole design rests on this file: the model never writes a resolution, it
 * only picks one of these. So this has to be generous enough that the right
 * answer is usually in the list, and strict enough that nothing in the list is
 * nonsense.
 *
 * The three-way merge below is generic over arrays, and gets used twice: once
 * on the lines of a hunk, and once on the tokens of a line. The token pass is
 * what catches the common case where both sides edited the same line in
 * compatible ways — "price * qty" and "price * (1 + tax)" want to become
 * "price * qty * (1 + tax)", and no line-level merge can see that.
 */
...
/**
 * Three-way merge of arrays. Returns a list of merged results — usually one,
 * two when both sides inserted at exactly the same point and the order is a
 * real choice, and none when the sides genuinely overlap.
 */
export function threeWay(base, ours, theirs) {
  ...
  while (a < ourR.length || b < theirR.length) {
    ...
    if (ra && rb) {
      ...
      // Two pure insertions at the same point. Both regions are zero-length,
      // so the disjointness tests below would wrongly call them separate —
      // this has to come first. Which order is right is a real question, so
